...
source('Fxns.R')
RANKL_UMIs<-load_data(data_name = './data/GSE92332_Org_RANKL_UMIcounts.txt.gz')
...
RANKL.tsne.rot<-PCA_TSNE.scores(data.tpm = RANKL_tpm_norm,data.umis = RANKL_UMIs,var_genes = var.genes,
                               data_name = './data/Org_RANK',sig.pcs = TRUE,is.var.genes = TRUE)
...
### Figure a
#control.color<-ifelse(RANKL.condition=='Control','red','white')
...
RANKL.marker.genes<-c('Anxa5','Ccl20','Ccl6','Ccl9','Gp2','Marcksl1','Pglyrp1','Rac2',
                      'Spib','Tnfaip2')
# for(g in RANKL.marker.genes){
#   if(!g%in%RANKL.all.genes){
#     cat(sprintf('%s\n',g))
#   }
# }

# RANKL.marker.tpm<-data.frame(t(RANKL_tpm_norm[RANKL.marker.genes,]))
# RANKL.marker.tpm$Groups<-RANKL.cell.groups
# 
# RANKL.marker.melt.tpm<-melt(RANKL.marker.tpm)
# p<-ggplot(data = RANKL.marker.melt.tpm,aes(x=Groups,y=value))+geom_violin(aes(colour=variable,fill=value),trim =TRUE)
...
# Violin_Genes(gene = 'Ccl20',tpm.data = RANKL_tpm_norm,cell.groups = RANKL.cell.groups,marker.genes = RANKL.marker.genes,
#              groups.color = Groups.color)

# for(gene in RANKL.marker.genes){
#   Violin_Genes(gene = gene,tpm.data = RANKL_tpm_norm,cell.groups = RANKL.cell.groups,marker.genes = RANKL.marker.genes,
#                groups.color = Groups.color)
#   
# }
...
ggplot(data=RANKL.marker.tpm.melt,aes(x=Cells,y=value))+
  geom_violin(aes(fill=value,colour=Cells))+
